...
class ServerSpec extends Specification with CatsEffect {
	...
	private val slowUri = uri"http://localhost:8080/slow/5/15/5"
	...
	"MyServerSpec" should {
		...
		"slow numbers" in mkEnv.flatMap(run(slowUri, _).map(_ must beEqualTo("5, 15, 5")))

		"no matches" in mkEnv.flatMap(run(badSlowUri, _).map(_ must beEqualTo("No matches")))
